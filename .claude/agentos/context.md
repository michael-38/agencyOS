...
Run: `cd website-audit && npm install && npx tsx src/index.ts audit <url>`; UI: `npm run ui`. Also: `validate-personas`, `list-checks`, `list-modules`, `eval`, `check-html`, `site:build` (SiteRedesign), `site:scaffold|validate|preview` (v2). Modules (all toggleable), two groups: the audit pipeline (classify, common-checklist, persona-checklist, deterministic, judgment, subpath, desktop, facts, lighthouse (reserved)) and OpenSEO enrichment, all off by default (openseo-crawl/search-console/analytics/project-context are free with an OpenSEO account; openseo-lighthouse/keywords/rankings/serp/competitors/backlinks/local-pack/local-grid/reviews/rank-tracking bill your DataForSEO key). The CLI cannot call MCP, so enrichment modules write pre-filled requests to report.json `openseo.requests[]` for an agent session to run — a run never spends DataForSEO credits itself.
