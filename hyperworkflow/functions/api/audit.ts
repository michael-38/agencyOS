...
// Reject control characters (incl. CR/LF) — defense against header/subject injection.
const hasControlChars = (s: string) => /[\x00-\x1F\x7F]/.test(s);

// Strip control characters but keep newlines and tabs — for free-form multi-line notes.
const sanitizeNotes = (s: string) =>
  s.replace(/\r\n/g, "\n").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "").trim();

const normalizeUrl = (raw: string): string | null => {
  if (raw.length > 2048 || hasControlChars(raw) || /\s/.test(raw)) return null;
  try {
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const websiteDisplay = new URL(url).hostname;
